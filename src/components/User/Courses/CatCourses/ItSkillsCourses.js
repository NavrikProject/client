    ...
    const getCourseByCatCourse = async () => {
      try {
        const response = await axios.get(
          searchItem
            ? `https://navriktrainingserverside.azurewebsites.net/api/courses/search?name=${searchItem}`
            : `https://navriktrainingserverside.azurewebsites.net/api/courses/category/it-skills`
        );
        if (!response.data) {
          return;
        }
