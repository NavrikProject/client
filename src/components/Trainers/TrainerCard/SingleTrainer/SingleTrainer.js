...
const SingleTrainer = () => {
    ...
    const getTrainerFullDetails = async () => {
      try {
        const result = await axios.get(
          `https://navriktrainingserverside.azurewebsites.net/api/trainer/profile/get/${path}`
        );
  ...
  useEffect(() => {
    const getOnlyTrainerCourses = async () => {
      try {
        const result = await axios.get(
          `https://navriktrainingserverside.azurewebsites.net/api/trainer/profile/courses/get/${path}`
        );
        if (!result.data) {
          return;
        }
