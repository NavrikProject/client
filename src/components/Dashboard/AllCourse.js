...
const AllCourse = () => {
  ...
  useEffect(() => {
    const getAllTheCourse = async () => {
      ...
      const res = await axios.get(
        `https://navriktrainingserverside.azurewebsites.net/api/courses/new/dashboard/courses`,
        {
  ...
  const courseEditHandler = async (course) => {
    // const res = await axios.put(`https://navriktrainingserverside.azurewebsites.net/api/courses/new/edit/${course.course_id}`, {
    //   headers: { authorization: "Bearer " + token },
    // });
    // console.log(res);
  };
