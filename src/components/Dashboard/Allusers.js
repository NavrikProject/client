...
const Allusers = () => {
  ...
  useEffect(() => {
    const getAllTheUsers = async () => {
      ...
      const res = await axios.get(
        `https://navriktrainingserverside.azurewebsites.net/api/users/get`,
        {
  ...
  const userAdminApproveHandler = async (user) => {
    ...
    const res = await axios.put(
      `https://navriktrainingserverside.azurewebsites.net/api/users/update/approve/${user.user_dtls_id}`,
  ...
  const userAdminDisApproveHandler = async (user) => {
    setLoading(true);
    const res = await axios.put(
      `https://navriktrainingserverside.azurewebsites.net/api/users/update/disapprove/${user.user_dtls_id}`,
      { id: user.user_dtls_id },
      {
        headers: { authorization: "Bearer " + token },
      }
