  ...
  const profileSubmitHandler = async (event) => {
    ...
    try {
      const res = await axios.post(
        `https://navriktrainingserverside.azurewebsites.net/api/trainee/profile/create/${user?.id}`,
        data,
  ...
  useEffect(() => {
    const checkTraineeDetails = async () => {
      const res = await axios.get(
        `https://navriktrainingserverside.azurewebsites.net/api/trainee/profile/check/${user?.id}`,
        {
          headers: { authorization: "Bearer " + token },
        }
      );
